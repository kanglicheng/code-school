...
* Make sure you are passing `@cat` into your partial as an attribute like so: `<%= render 'form', cat: @cat %>`
  * We do not want to use instance variables in our partials because it promotes **coupling** between our partial and the views that render them. By passing in the instance variable as an attribute, we keep our code DRY and prevent bugs should we ever decide to refactor our code. 
